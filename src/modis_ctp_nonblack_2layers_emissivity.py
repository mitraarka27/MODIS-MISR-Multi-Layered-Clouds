# -*- coding: utf-8 -*-
"""
Created on Wed Apr 15 19:59:29 2020
...
Ics = np.zeros(4)
intensity_at_level=np.zeros((len(Pres)-40,20))
ratio_at_level=np.zeros((len(Pres)-40,4,20))
intensity_at_level[:,4]=[Pres[j] for j in range(20,len(Pres)-20)]
trial_pressure=np.zeros((1000,100,10))
trial_ratio=np.zeros((1000,100,6))
band_snr=[0.75,1.0,1.0,1.25]
eff_am=np.linspace(0.05,1.0,20)
...
trial_sat_pressure=np.zeros((1000,100,24,20))
trial_sat_ratio=np.zeros((1000,100,3,20))
...
randp = np.linspace(np.max(sat_radiances[:,4,19]), np.min(sat_radiances[:,4,19]), 1000) #Random upper CTP
...
for i in range(0,len(randp)):
    print("i=",i)
    for nAmount in range(0,20):
        trial_sat_pressure[i,:,8,nAmount] = randp[i]

    randlow = np.linspace(np.min(sat_radiances[:,4,19]), randp[i], 100)
           
    for nAmount in range(0,20):
        trial_sat_pressure[i,:,10,nAmount] = randlow


    #Approximate cloudy radiance at trial upper & lower pressure levels
    trial_sat_pressure[i,:,0,19]=f1(trial_sat_pressure[i,:,8,19])
    trial_sat_pressure[i,:,1,19]=f2(trial_sat_pressure[i,:,8,19])
    trial_sat_pressure[i,:,2,19]=f3(trial_sat_pressure[i,:,8,19])
    trial_sat_pressure[i,:,3,19]=f4(trial_sat_pressure[i,:,8,19])
    trial_sat_pressure[i,:,4,19]=f1(trial_sat_pressure[i,:,10,19])
    trial_sat_pressure[i,:,5,19]=f2(trial_sat_pressure[i,:,10,19])
    trial_sat_pressure[i,:,6,19]=f3(trial_sat_pressure[i,:,10,19])
    trial_sat_pressure[i,:,7,19]=f4(trial_sat_pressure[i,:,10,19])
    ...
    for nAmount in range(0,len(eff_am)):
        ...
        noise = np.random.normal(0, 0.25, (1000,100))
                ...
                try:
                    ...
                    trial_sat_pressure[ntrial,lcld,wv+20,nAmount]= \
                        (trial_sat_pressure[ntrial,lcld,wv+11,nAmount]- \
                         trial_sat_pressure[ntrial,lcld,5,nAmount])
                except:
...
for wv in range(0,3):
    for nAmount in range(0,20,5):   
        for nlevel in range(0,1000):
            ...
            for lcld in range(0,100):
                ...
                if (np.isnan(trial_sat_pressure[nlevel,lcld,wv+11,nAmount])==False):
                           bias[m,n,int(nAmount/5),wv] = bias[m,n,int(nAmount/5),wv] + \
                           (trial_sat_pressure[m,n,wv+11,nAmount]- \
                                  trial_sat_pressure[m,n,8,nAmount])
                           c[m,n,int(nAmount/5),wv] = c[m,n,int(nAmount/5),wv] + 1

        bias[:,:,int(nAmount/5),wv] = np.divide(bias[:,:,int(nAmount/5),wv], c[:,:,int(nAmount/5),wv]+1, \
            out=np.zeros_like(bias[:,:,int(nAmount/5), wv]), where=c[:,:,int(nAmount/5), wv]!=0)

##
fig, axes = plt.subplots(nrows = 3, ncols=4, figsize=(16,10))
for wv in range(0,3):
    for i in range(0,20,5):
        ax=axes.flat[4*wv+int(i/5)]   
        im=ax.imshow(bias[:,:,int(i/5),wv], cmap='seismic', vmin=-400., vmax=400.)
...
plt.savefig('./plots/MODIS_CO2bands_2layers_ratio1_bias_ctp_1.png', dpi=400, bbox_to_inches='tight')
plt.show()
...
plt.savefig('./plots/MODIS_CO2bands_2layers_ratio1_rmse_ctp_1.png', dpi=400, bbox_to_inches='tight')
plt.show()
...
for i in range(0,3):
    ...
    ax.plot(P,-errorbylow[:,1,i],label='$\epsilon A_c = $'+str(eff_am[5]));
    ax.plot(P,-errorbylow[:,2,i],label='$\epsilon A_c = $'+str(eff_am[10]));
    ax.plot(P,-errorbylow[:,3,i],label='$\epsilon A_c = $'+str(eff_am[15]));
    ...
    if (i==0):
        ax.set_ylabel('Underestimation of top layer \n cloud-top pressure (hPa)', fontsize=13);
plt.savefig('./plots/MODIS_CO2bands_2layers_bias_bylowCTP_ctp_1.png', dpi=400, bbox_to_inches='tight')
plt.show()
